<template>
  <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
      <div>
        <h2 class="text-xl font-semibold text-text-primary tracking-tight">Results</h2>
        <p class="text-sm text-text-secondary mt-1" v-if="results.length">{{ results.length }} photos analyzed</p>
      </div>
    ...
    <div v-else-if="results.length === 0" class="rounded-xl border border-border-default bg-surface-1 p-12 text-center">
      ...
      <p class="text-sm text-text-secondary mb-3">No results yet</p>
          ...
          <div class="relative bg-surface-1 border border-border-default rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden shadow-2xl">
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(85vh-100px)]">
              ...
              <div v-if="selected.keywords?.length" class="mb-5">
                <p class="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-2">Keywords</p>
              ...
              <div class="grid grid-cols-2 gap-3 mb-5">
                <div class="bg-surface-2 rounded-lg p-3">
                  <p class="text-[10px] font-medium text-text-tertiary uppercase tracking-wider mb-1">Category</p>
                  <p class="text-sm font-medium text-text-primary">{{ selected.category || '--' }}</p>
                </div>
                <div class="bg-surface-2 rounded-lg p-3">
                  <p class="text-[10px] font-medium text-text-tertiary uppercase tracking-wider mb-1">Scene</p>
                  <p class="text-sm font-medium text-text-primary">{{ selected.scene_type || '--' }}</p>
                </div>
                <div class="bg-surface-2 rounded-lg p-3">
                  <p class="text-[10px] font-medium text-text-tertiary uppercase tracking-wider mb-1">Mood</p>
                  <p class="text-sm font-medium text-text-primary">{{ selected.mood || '--' }}</p>
                </div>
                <div class="bg-surface-2 rounded-lg p-3">
                  <p class="text-[10px] font-medium text-text-tertiary uppercase tracking-wider mb-1">People</p>
              ...
              <div v-if="selected.identified_people?.length" class="mb-5">
                <p class="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-2">Identified People</p>
                <p class="text-sm text-text-secondary">{{ selected.identified_people.join(', ') }}</p>
              </div>

              <!-- OCR -->
              <div v-if="selected.ocr_text?.length" class="mb-5">
                <p class="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-2">OCR Text</p>
                <p class="text-sm text-text-secondary font-mono bg-surface-2 rounded-lg p-3">{{ selected.ocr_text.join(' | ') }}</p>
              </div>

              <!-- File path -->
              <div class="pt-4 border-t border-border-default">
                <p class="text-[10px] font-medium text-text-tertiary uppercase tracking-wider mb-1">File Path</p>
